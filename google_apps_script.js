/**
 * LUXUS BACKEND v27.0 - COMPLETO E TESTADO
 */
  ...
  try {
    ...
    if (action === 'addInventario') {
      ...
      sheet.appendRow([
        ...
        parseFloat(params.custo) || 0,
        parseFloat(params.venda) || 0,
        params.foto || '',
        params.tipo || '',
        params.descricao || ''
      ]);
    ...
    if (action === 'addRepasse') {
      ...
      sheetRep.appendRow([rev, cod, parseFloat(params.custo) || 0, parseFloat(params.venda) || 0, params.data || new Date(), 'Pendente']);
    ...
    if (action === 'addRevendedor') {
      ...
      sheet.appendRow([params.nome?.toString().trim(), params.contato || '', parseFloat(params.comissao) || 30]);
    ...
    if (action === 'editRevendedor') {
      ...
      for (let i = 1; i < data.length; i++) {
        if (data[i][0] && data[i][0].toString().trim() === nomeAntigo) {
          ...
          sheet.getRange(i + 1, 3).setValue(parseFloat(params.comissao) || 30);
