...
function doGet(e) {
  ...
  try {
    ...
    if (action === 'getUsuarios') return createCORSResponse(getSheetAsJSON(ss, 'Usuarios'));
...
function getSheetAsJSON(ss, sheetName) {
  ...
  const json = rows.map((row, index) => {
    ...
    if (sheetName === 'Usuarios') { obj.Nome = row[0]; obj.Senha = row[1]; }
    return obj;
  });
  return json;
}
...
function doPost(e) {
  ...
  if (action === 'editInventario') {
    ...
    if (rowId) {
      if (params.codigo) sheet.getRange(rowId, 1).setValue(params.codigo.toString().trim());
      if (params.status) sheet.getRange(rowId, 3).setValue(params.status);
      if (params.custo !== undefined) sheet.getRange(rowId, 4).setValue(params.custo);
      if (params.venda !== undefined) sheet.getRange(rowId, 5).setValue(params.venda);
